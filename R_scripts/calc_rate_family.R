...
rate.data <- d %>%
  ...
  summarise(nmut=sum(!is.na(ts)),
            ...
            nPat = sum(POO=='P'),
            nMat = sum(POO=='M'),
            nPat2 = sum(POO=='P' & POO_LR > 3),
            nMat2 = sum(POO=='M' & POO_LR < -3.0),
            nRepeat = sum(repeat.),
            nNonRepeat = sum(repeat.==FALSE)) %>%
  merge(fam.info) %>%
  as.data.frame()
  #mutate(rate=nmut/(2*SNV.SUM*(1-alpha.site)),
  #       se=sqrt((rate*(1-rate))/(2*SNV.SUM*(1-alpha.site))),
  #       tstv_ratio=n.ts/n.tv) %>% as.data.frame()
